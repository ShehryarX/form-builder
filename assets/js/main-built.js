var extendedArray = []; // Checks whether an element that is extended is still extended and not deleted`
var extendThisTabJSONString = []; // Keeps track of the JSON of the extended objects (to render)
var extendThisHandlerIDJSON = []; // Keeps track of the JSON ID assigned to each extended item
var firstExtendDrop = []; // Checks the length of the extended tabs
var globalExtendThisTabHolder = []; // Keeps the index of each respsective extend tab
var extendTabNames = []; // Keeps track of extended tab names
var jsonData; // Used to export data through AJAX request
var firstLoad = 0; // Keeps track of the first success load of the page
...
function() {
    ...
    function() {
        ...
        s.extend(b.prototype, a, {
            ...
            setElement: function(e, t) {
                return this.$el && this.undelegateEvents(),
                    ...
                    this.el = this.$el[0], t !== !1 && this.delegateEvents(), this
            },
        ...
        return '<form class=\'form\'>\n  <div class=\'controls\'>\n    <% var compiled =  _.reduce(items, function(str, v, k){ %>\n      <% v["name"] = k; %>\n      <% return str + popoverTemplates[v["type"]](v); %>\n    <% }, "") %>\n    <%= compiled %>\n    <hr/>\n    <button id="save" class=\'btn btn-primary\'>Save</button><button id="extendThis" class=\'btn btn-info\'>Extend</button><button id="cancel" class=\'btn btn-warning\'>Cancel</button><button id="deleteThis" class=\'btn btn-danger\'>Delete</button>\n</div>\n</form>\n'
    }), define("text!templates/popover/popover-input.html", [], function() {
        return "<label class='control-label'> <%= label %> </label>\n<input class='input-large field form-control' data-type=\"<%= type %>\" type='text' name='<%= name %>' id='<%= name %>' value ='<%= value %>' />\n"
    }), define("text!templates/popover/popover-select.html", [], function() {
        return '<label class=\'control-label\'> <%= label %> </label>\n<select class="field form-control" data-type="<%= type %>" id=\'<%= name %>\'>\n<% _.each(value, function(v,k){ %>\n  <option value="<%= v["value"] %>" <% if (v["selected"]){ %> selected <% } %> ><%= v["label"] %></option>\n<% }); %>\n</select>\n'
    }), define("text!templates/popover/popover-textarea.html", [], function() {
        return "<label class='control-label'> <%= label %> </label>\n<textarea class=\"field form-control\" data-type=\"<%= type %>\" style='min-height: 200px' id='<%= name %>'><%= value %></textarea>\n"
    }), define("text!templates/popover/popover-textarea-split.html", [], function() {
        return "<label class='control-label'> <%= label %> </label>\n<textarea class=\"field form-control\" data-type=\"<%= type %>\" style='min-height: 200px' id='<%= name %>'><% for ( var i = 0; i < value.length ; i++ ) { %><%= value[i] %><% if (i < value.length - 1) { %><%= \"\\n\" %><% } %><% } %></textarea>\n"
        ...
        return '<!-- Password input-->\n<div class="form-group<% if(required) {%> required-control<% } %>">\n  <label  class="col-md-3 control-label" for="<%= id %>"><%= label %></label>\n  <div class="<%= inputsize %>">\n    <input id="<%= id %>" name="<%= id %>" type="password" placeholder="<%= placeholder %>" class="form-control <%= inputheight %>" <% if(required) {%> required <% } %> />\n    <% if (helptext.length > 0) { %><p class="help-block"><%= helptext %></p><% } %>\n  </div>\n</div>\n'
        ...
        return '<!-- Select Basic -->\n<div class="form-group">\n  <label  class="col-md-3 control-label" for="<%= id %>"><%= label %></label>\n  <div class="<%= inputsize %>">\n    <select id="<%= id %>" name="<%= id %>" class="form-control <%= inputheight %>"><% _.each(options, function(option) { %>\n      <option><%= option %></option><% }); %>\n    </select>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/selectmultiple.html", [], function() {
        return '<!-- Select Multiple -->\n<div class="form-group">\n  <label class="col-md-3 control-label" for="<%= id %>"><%= label %></label>\n  <div class="<%= inputsize %>">\n    <select id="<%= id %>" name="<%= id %>" class="form-control <%= inputheight %>" multiple="multiple"><% _.each(options, function(option) { %>\n      <option><%= option %></option><% }); %>\n    </select>\n  </div>\n</div>\n'
    }), define("text!templates/snippet/textarea.html", [], function() {
        return '<!-- Textarea -->\n<div class="form-group<% if(required) {%> required-control<% } %>">\n  <label class="col-md-3 control-label" for ="<%= id %>"><%= label %></label>\n  <div class="<%= inputsize %>">\n    <textarea id="<%= id %>" name="<%= id %>" <% if(required) {%> required <% } %> class="form-control <%= inputheight %>"><%= textarea %></textarea>\n  </div>\n</div>\n'
...
if (!jQuery) throw new Error("Bootstrap requires jQuery"); + function(e) {
    ...
    return r.extend({
        ...
        mouseDownHandler: function(t) {
            ...
            var n = this;
            e(".popover").remove(), this.$el.popover("show"),
                e(".popover #save").on("click", this.saveHandler(n)),
                e(".popover #extendThis").on("click", this.extendThisHandler(n)),
                e(".popover #deleteThis").on("click", this.deleteThisHandler(n)),
                e(".popover #cancel").on("click", this.cancelHandler(n)),
                this.model.get("title") !== "Form Name" && e("body").on("mousemove", function(e) {
        ...
        extendThisHandler: function(n2) {
            return function(r) {
                var optionBuilder = "";
                for (var i = 0; i < extendTabNames.length; i++)
                    optionBuilder += "<option>" + extendTabNames[i] + "</option>";

                document.getElementById('extendOptions').innerHTML = optionBuilder;
                ...
                document.getElementById('extendOptionsConfirm').onclick = function() {
                    choice = document.getElementById('extendFinder').value;

                    if (!(choice == '') && !(choice.length <= 1)) {
                        document.getElementById('extendFinder').value = '';
                        r.preventDefault();
                        extendedArray.push(n2.model.cid);

                        choice = choice.replace(/\s+$/, '');

                        if (extendTabNames.indexOf(choice) < 0) {
                            extendTabNames.push(choice);
                            extendThisTabJSONString.push("[]");
                            firstExtendDrop.push(0);
                            globalExtendThisTabHolder.push("");
                            extendThisHandlerIDJSON.push(-1);
                        }

                        var indexValue = extendTabNames.indexOf(choice);
                        extendThisHandlerIDJSON[indexValue] += 1;

                        var tempJSON = JSON.stringify(n2);
                        tempJSON = tempJSON.substring(tempJSON.indexOf('\"model\"') + 8, tempJSON.indexOf('}}},')) + "}}" + ",\"extendID\":\"" + choice + " " + extendThisHandlerIDJSON[indexValue] + "\"" + "}";
                        if (firstExtendDrop[indexValue] == 0) {
                            extendThisTabJSONString[indexValue] = "[" + tempJSON + "]";
                            firstExtendDrop[indexValue]++;
                        } else {
                            extendThisTabJSONString[indexValue] = extendThisTabJSONString[indexValue].substring(0, extendThisTabJSONString[indexValue].length - 2) + "}, " + tempJSON + "]";
                            firstExtendDrop[indexValue]++;
                        }

                        for (var i = 0; i < extendTabNames.length; i++) {
                            $('#formtabs li:contains(' + extendTabNames[i] + ')').remove();
                        }

                        define("app/app", ["jquery", "underscore", "backbone", "collections/snippets", "collections/my-form-snippets", "views/tab", "views/my-form", "text!data/input.json", "text!data/radio.json", "text!data/select.json", "text!data/buttons.json", "text!templates/app/render.html", "text!templates/app/about.html", "text!data/extend.json"], function(e, t, n, r, i, s, o, u, a, f, l, c, h, d) {
                            return {
                                initialize: function() {
                                    try {
                                        for (var i = 0; i < globalExtendThisTabHolder.length; i++) {
                                            globalExtendThisTabHolder[i].remove();
                                            globalExtendThisTabHolder[i].unbind();
                                        }
                                    } catch (err) {}
                                    for (var i = 0; i < globalExtendThisTabHolder.length; i++) {
                                        globalExtendThisTabHolder[i] = new s({
                                            title: extendTabNames[i],
                                            collection: new r(JSON.parse(extendThisTabJSONString[i]))
                                        })
                                    }
                                }
                            }
                            ...
                            e.initialize();
                            $('a[href="#' + extendTabNames[indexValue] + '"]').click();
                        }), define("../main", function() {});
                        n2.model.trigger("myField", n2);
                        e(".popover").remove();
                        modal.style.display = "none";

                    } else {
                        alert("Please enter a tab name greater than or equal to two characters.");
                    }
                }
        ...
        deleteThisHandler: function(n2) {
            return function(r) {
                e(".popover").remove(),
                    n2.model.trigger("deleteSnippet", n2)
            }
        },
    ...
    return n.View.extend({
        ...
        initialize: function() {
            this.collection.on("add", this.render, this),
                ...
                this.collection.on("myField", this.extendThis, this),
                this.collection.on("deleteSnippet", this.deleteThisSnippet, this),
                ...
                this.$build = e("#build"),
                this.renderForm = t.template(s), this.render()
        },
        toJSON: function() {
            jsonData = JSON.stringify(this.collection.toJSON());
            document.getElementById("renderjson").innerHTML = jsonData;
            //console.API;
        ...
        deleteThisSnippet: function(n) {
            var i = 0;
            var collectionArray = n.model.collection.models; // Iterate through collection to find the right element cid

            for (i = 0; i < collectionArray.length; ++i)
                if (n.model.cid === collectionArray[i].cid) // When the right cid is reached, exit for loop
                    break;

            this.collection.at(i).destroy(); // Destroy element at the respective cid corresponding to the index, i
        },
        extendThis: function(n) {
                ...
                if (name === "New Form")
                    ...
                    $.ajax({
                        ...
                        error: function(response) {
                            console.log("Failed to load JSON data from file.");
                        }
                    });
                }


                console.log(JSON.parse(newJSON[1]));  
        ...
        handleTempMove: function(t) {
            ...
            t.pageX >= this.$build.position().left && t.pageX < this.$build.width() + this.$build.position().left && t.pageY >= this.$build.position().top && t.pageY < this.$build.height() + this.$build.position().top ? e(this.getBottomAbove(t.pageY)).addClass("target") : e(".target").removeClass("target")
        },
        handleTempDrop: function(t, n, r) {
            if ($.inArray(n.cid, extendedArray) > -1) {
                var r = e(".target").index();
                if (r != -1) {
                    e(".target").removeClass("target");
                    this.collection.add(n.clone(true, true), {
                        at: r + 1
                    });
                }
            } else {
                ...
                } else if (t.pageX >= window.innerWidth * 9 / 10) { // check position of mouse (relative, no hard coding!)
                    var extendReferenceID = n["attributes"]["extendID"];
                    if (extendReferenceID != null) {
                        var splitExtendReferenceID = extendReferenceID.split(" ");
                        var nameOfExtendedTab = splitExtendReferenceID[0];
                        var indexOfExtendedTab = parseInt(splitExtendReferenceID[1]);
                        var indexValue = extendTabNames.indexOf(nameOfExtendedTab);
                        var extendReferenceIDJSON = extendThisTabJSONString[indexValue];
                        var temp = JSON.parse(extendReferenceIDJSON);
                        temp.splice(indexOfExtendedTab, 1);

                        for (var i = indexOfExtendedTab; i < Object.keys(temp).length; i++) {
                            var tempSplit = temp[i]["extendID"].split(" ");
                            var d = parseInt(tempSplit[1]);
                            d--;
                            temp[i]["extendID"] = tempSplit[0] + " " + d;
                        }

                        extendThisTabJSONString[indexValue] = JSON.stringify(temp);
                        firstExtendDrop[indexValue]--;

                        // Updates indices of the rest of the values
                        for (var i = 0; i < extendTabNames.length; i++) {
                            $('#formtabs li:contains(' + extendTabNames[i] + ')').remove();
                        }
                        ...
                        define("app/app", ["jquery", "underscore", "backbone", "collections/snippets", "collections/my-form-snippets", "views/tab", "views/my-form", "text!data/input.json", "text!data/radio.json", "text!data/select.json", "text!data/buttons.json", "text!templates/app/render.html", "text!templates/app/about.html", "text!data/extend.json"], function(e, t, n, r, i, s, o, u, a, f, l, c, h, d) {
                            return {
                                initialize: function() {
                                    ...
                                    if (firstExtendDrop[indexValue] < 1) {
                                        extendThisHandlerIDJSON[indexValue] = -1;
                                        firstExtendDrop[indexValue] = 0;
                                    }
...
$('#saveJSONButton').on('click', function(e) {
    $.ajax({
        type: 'POST',
        url: 'savejson.php',
        data: {
            'data': jsonData,
            'name': document.getElementById("formname").innerHTML,
            'extendedData': extendThisTabJSONString,
            'extendedFilenames': extendTabNames
        },
        success: function(response) {
            console.log("Saved JSON locally.");
...
$('#loadJSONButton').on('click', function(e) {
    /*$.ajax({
      type: 'POST',
      url: 'loadjson.php',
      data: {},
...
$(document).ready(function() {
    if (firstLoad == 0) {
        ...
        $.ajax({
            ...
            $("#loadJSONOptions").append(html);
        });

        var choice = "";
        var modal = document.getElementById('loadQuery');
        modal.style.display = "block";

        document.getElementById('loadJSONOptionConfirm').onclick = function() {
            choice = (document.getElementById('loadJSONText').value);
            ...
            define("text!data/input.json", [], function() {
                ...
                return '[ { "title": "File Button", "fields": { "id": { "label": "ID / Name", "type": "input", "value": "filebutton" }, "label": { "label": "Label Text", "type": "input", "value": "File Button" } } }, { "title": "Single Button", "fields": { "id": { "label": "ID / Name", "type": "input", "value": "singlebutton" }, "label": { "label": "Label Text", "type": "input", "value": "Single Button" }, "buttonlabel": { "label": "Button Label", "type": "input", "value": "Button" }, "buttontype": { "label": "Button Type", "type": "select", "value": [ { "value": "btn-default", "label": "Default", "selected": false }, { "value": "btn-primary", "label": "Primary", "selected": true }, { "value": "btn-info", "label": "Info", "selected": false }, { "value": "btn-success", "label": "Success", "selected": false }, { "value": "btn-warning", "label": "Warning", "selected": false }, { "value": "btn-danger", "label": "Danger", "selected": false } ] } } }, { "title": "Double Button", "fields": { "id": { "label": "Button 1 ID / Name", "type": "input", "value": "button1id" }, "label": { "label": "Label Text", "type": "input", "value": "Double Button" }, "button1label": { "label": "Button Label", "type": "input", "value": "Good Button" }, "button1type": { "label": "Button Type", "type": "select", "value": [ { "value": "btn-default", "label": "Default", "selected": false }, { "value": "btn-primary", "label": "Primary", "selected": false }, { "value": "btn-info", "label": "Info", "selected": false }, { "value": "btn-success", "label": "Success", "selected": true }, { "value": "btn-warning", "label": "Warning", "selected": false }, { "value": "btn-danger", "label": "Danger", "selected": false } ] }, "id2": { "label": "Button 2 ID / Name", "type": "input", "value": "button2id" }, "button2label": { "label": "Button Label", "type": "input", "value": "Scary Button" }, "button2type": { "label": "Button Type", "type": "select", "value": [ { "value": "btn-default", "label": "Default", "selected": false }, { "value": "btn-primary", "label": "Primary", "selected": false }, { "value": "btn-info", "label": "Info", "selected": false }, { "value": "btn-success", "label": "Success", "selected": false }, { "value": "btn-warning", "label": "Warning", "selected": false }, { "value": "btn-danger", "label": "Danger", "selected": true } ] } } } ]'
            }), define("text!data/extend.json", [], function() {
                return extendThisTabJSONString;
            }), define("text!templates/app/render.html", [], function() {
                return '<h3>Rendered source of your form:</h3>\n<textarea id="render" class="col-md-6 form-control"></textarea>\n'
            }), define("text!templates/app/extend.html", [], function() {
                return '<div class="col-md-14"><div class="clearfix"><h3>Extended snippets</h3><hr><div id="extendThisField"><form id="extendThisFieldTarget" class="form-horizontal"></form></div></div></div>'
            }), define("text!templates/app/about.html", [], function() {
                return '<h3>Rendered source of your form:</h3><p><i>See <b>latest</b> console log for a more readable version of the JSON. (Ctrl + Shift + J)</i></p><textarea rows="10" cols="70" id="renderjson" class="span6"></textarea><center>'
            }), define("app/app", ["jquery", "underscore", "backbone", "collections/snippets", "collections/my-form-snippets", "views/tab", "views/my-form", "text!data/input.json", "text!data/radio.json", "text!data/select.json", "text!data/buttons.json", "text!templates/app/render.html", "text!templates/app/about.html", "text!data/extend.json"], function(e, t, n, r, i, s, o, u, a, f, l, c, h, d) {
                return {
                    initialize: function() {
                        new s({
                            ...
                            e("#formtabs li").first().addClass("active"),
                            e("#toggle_bs_style").on("click", function(t) {
                                t.preventDefault(), e("#bootstrap-classic-theme").attr("href") == "" ? e("#bootstrap-classic-theme").attr("href", "assets/css/lib/bootstrap-3.0.0/dist/css/bootstrap-theme.min.css") : e("#bootstrap-classic-theme").attr("href", "")
                            }), new o({
                                title: "Original",
                                collection: new i([{
                                    title: "Form Name",
